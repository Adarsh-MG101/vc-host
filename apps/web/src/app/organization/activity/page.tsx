...
import { AdminActivity, ActivityFilterOptions } from '../../../services/superadmin.service';
...
export default function OrganizationActivityPage() {
  ...
  const fetchActivity = useCallback(async () => {
    ...
    try {
      ...
      const response = await organizationService.getActivities(options);
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Timestamp', 'User', 'Email', 'Action', 'IP Address', 'Browser'];
    ...
    link.setAttribute('download', `org_activity_log_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="flex flex-col space-y-6">
      ...
      >
        <div className="space-y-1">
          ...
          <p className="text-xs text-gray-400 font-medium ml-[40px]">Audit trail for all actions within your organization</p>
        </div>
      ...
      >
        ...
        <div className="flex items-center space-x-4 pt-2 border-t border-gray-50 mt-2">
          ...
          {(startDate || endDate || selectedType !== 'all' || search) && (
            <button 
              onClick={() => {
                setSearch('');
                setSelectedType('all');
